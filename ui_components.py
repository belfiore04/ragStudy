...
def render_evidence_cards(proj, hits: List[Document]):
    ...
    for d in hits:
        ...
        with st.expander(f"{tag} 路 " + (f"P{page}" if page else (f"S{slide}" if slide else ""))):
...
def render_mcq_block(proj, data: Dict[str, Any], qid: str):
    ...
    sel = st.radio("閫夐」", opts, key=choice_key)
    c1, c2 = st.columns(2)
    if c1.button("鎻愪氦", key=submit_key):
    ...
    if fb:
        if fb["your"] == fb["correct"]:
            ...
            st.error(f"閿欒銆傜瓟妗堬細{fb['correct']}")
            if not fb.get("logged"):
                try:
                    ...
                    st.warning(f"鍐欏叆閿欓鏈け璐ワ細{e}")
    with st.expander("绛旀涓庤В鏋�"):
